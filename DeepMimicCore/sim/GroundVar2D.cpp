...
void cGroundVar2D::tSegment::Init(const std::shared_ptr<cWorld>& world, double min_x, double friction)
{
	...
	mColShape = std::unique_ptr<btCollisionShape>(height_field);
...
tVector cGroundVar2D::tSegment::GetScaling() const
{
	...
	btVector3 bt_scale = mColShape->getLocalScaling();
...
tVector cGroundVar2D::tSegment::GetSize() const
{
